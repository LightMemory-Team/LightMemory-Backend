...
def build_questions():
    return (
        [{"question_index": i + 1, "is_correct": True, "reaction_time_ms": 2000,
          "trial_type": "repeat", "error_type": None} for i in range(5)]
        + [{"question_index": 6, "is_correct": False, "reaction_time_ms": 2500,
            "trial_type": "switch", "error_type": "persistent"}]
        + [{"question_index": 7, "is_correct": True, "reaction_time_ms": 2100,
            "trial_type": "switch", "error_type": None}]
        + [{"question_index": i, "is_correct": True, "reaction_time_ms": 2000,
            "trial_type": "repeat", "error_type": None} for i in range(8, 29)]
    )
...
class MarketSortSubmitEndpointTests(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='market_sort_tester', password='Str0ng!Pass2026')
        self.client.force_authenticate(user=self.user)

    def test_submit_complete_session_returns_201(self):
        payload = {"session_id": "test_1", "is_complete": True, "questions": build_questions()}
    ...
    def test_first_time_player_has_empty_recent_scores(self):
        payload = {"session_id": "test_2", "is_complete": True, "questions": build_questions()}
        response = self.client.post(SUBMIT_URL, payload, format='json')

        self.assertEqual(response.data['data']['recent_scores'], [])

    def test_incomplete_session_does_not_calculate_score(self):
        payload = {"session_id": "test_3", "is_complete": False, "questions": build_questions()[:10]}
        response = self.client.post(SUBMIT_URL, payload, format='json')
    ...
    def test_invalid_trial_type_returns_400(self):
        ...
        payload = {"session_id": "test_4", "is_complete": True, "questions": bad_questions}
        response = self.client.post(SUBMIT_URL, payload, format='json')
    ...
    def test_unauthenticated_request_rejected(self):
        ...
        payload = {"session_id": "test_5", "is_complete": True, "questions": build_questions()}
        response = self.client.post(SUBMIT_URL, payload, format='json')
